...
public class BossTankLogic : MonoBehaviour
{
        ...
        switch (currentState)
        {
            case bossStates.shooting:
                ...
                if (shotCounter <= 0)
                {
                    ...
                    var newBullet = Instantiate(bullet, shotPosition.position, shotPosition.rotation);
    ...
    public void TakeHit()
    {
        ...
        BossTankMineLogic[] mines = FindObjectsOfType<BossTankMineLogic>();
        if (mines.Length > 0)
        {
            foreach (BossTankMineLogic foundMine in mines)
            {
                foundMine.Explode();
            }
        }
